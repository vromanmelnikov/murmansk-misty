    ...
    if serverSMTP is not None:
        res = post(
            ...
            template.render(
                ...
                reason="запросили сброс пароля для своей учетной записи Erbium.",
    ...
    if serverSMTP is not None:
        res = post(
            ...
            template.render(
                logo=settings.LOGO_URL,
                header=f"{WARNING}!",
                reason="кто-то вошел на ваш аккаунт на платформе Erbium.",
                company_name=COMPANY_NAME,
            ),
            SUBJECT_SIGNIN,
            settings.PASSWORD,
...
@app.get("/greeting/")
async def root(email: EmailStr, name: str | None = Query(default=None)):
    html = open(settings.PATH_TEMPLATE, encoding="utf-8").read()
    template = Template(html)
    if serverSMTP is not None:
        res = post(
            ...
            template.render(
                ...
                reason="вы зарегистрировались на платформе Erbium.",
                company_name=COMPANY_NAME,
            ),
            SUBJECT_GREETING,
            settings.PASSWORD,
    ...
    if serverSMTP is not None:
        res = post(
            ...
            template.render(
                logo=settings.LOGO_URL,
                header="Уведомление!",
                reason="вам пришло сообщение с платформы Erbium.",
                message=message,
                company_name=COMPANY_NAME,
            ),
            MESSAGE,
